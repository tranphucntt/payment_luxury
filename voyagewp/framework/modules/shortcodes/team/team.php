...
class Team implements ShortcodeInterface {
	...
	public function vcMap() {
		...
		for($x = 1; $x < 6; $x++) {
			$teamIconCollections = voyage_mikado_icon_collections()->getCollectionsWithSocialIcons();
			...
			$team_social_icons_array[] = array(
				...
				'dependency' => array(
					...
					'value'   => voyage_mikado_icon_collections()->getIconCollectionsKeys()
				)
			);
			...
			'params'                    => array_merge(
				array(
					...
					array(
						...
						'admin_label' => true,
						'value'       => array_merge(array('' => ''), voyage_mikado_icon_collections()->getIconCollectionsVCExclude('linea_icons')),
						'save_always' => true
					),
					array(
						'type'        => 'dropdown',
	...
	public function render($atts, $content = null) {
		...
		$html = voyage_mikado_get_shortcode_module_template_part('templates/'.$params['team_type'], 'team', '', $params);
	...
	private function getTeamSocialIcons($params) {
		...
		if($team_social_icon_pack !== '') {
			...
			$icon_pack                    = voyage_mikado_icon_collections()->getIconCollection($team_social_icon_pack);
			...
			for($i = 1; $i <= $number_of_social_icons; $i++) {
				...
				if($team_social_icon !== '') {
					...
					$social_icons[] = voyage_mikado_execute_shortcode('mkdf_icon', $team_icon_params);
				}
