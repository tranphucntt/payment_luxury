<?php global $voyage_mikado_Framework; ?>

<div class="mkdf-tabs-navigation-wrapper">
    <ul class="nav nav-tabs">
        <?php
        foreach ($voyage_mikado_Framework->mkdOptions->adminPages as $key => $page ) {
            $slug = "";
            if (!empty($page->slug)) $slug = "_tab".$page->slug;
            ?>
            <li<?php if ($page->slug == $tab) echo " class=\"active\""; ?>>
                <a href="<?php echo esc_url(get_admin_url().'admin.php?page=voyage_mikado_theme_menu'.$slug); ?>">
                    <?php if($page->icon !== '') { ?>
        ...
        ?>
        <?php if (voyage_mikado_core_installed()) { ?>
        <li <?php if($isImportPage) { echo "class='active'"; } ?>><a href="<?php echo esc_url(get_admin_url().'admin.php?page=voyage_mikado_theme_menu_tabimport'); ?>"><i
